...
def class_titles_bar_chart(dataset, pass_class, cpalette=palette):
    ...
    p = figure(tools=[hover_tool], plot_height=400, title='Titles for Current Class')
    p.vbar(x='titles_int', top='values', source=source, width=0.9,
           fill_color=factor_cmap('titles', 
                palette=palette_generator(len(source.data['titles']), cpalette), 
